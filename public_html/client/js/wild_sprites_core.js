/**
 * Shared wild sprite art (8x8 source) and factory for 8 / 16 / 32 px grid variants.
 */
var AnimasterWildSpritesFactory = (function ()
{
    var SOURCE_ARCH = {
        dog: [
            '........',
    ...
    function inferGridSize(arch)
    {
        var keys = Object.keys(arch);

        if (!keys.length || !arch[keys[0]] || !arch[keys[0]].length)
        {
            return 8;
        }

        return arch[keys[0]][0].length;
    }
    ...
    function create(config)
    {
        ...
        var ARCH = config.arch || SOURCE_ARCH;
        var gridSize = inferGridSize(ARCH);
        var defaultPixelSize = config.defaultPixelSize !== undefined
            ? config.defaultPixelSize
            : Math.max(1, Math.round(16 / gridSize));
        var nearPixelSize = config.nearPixelSize !== undefined
            ? config.nearPixelSize
            : Math.max(1, defaultPixelSize + 1);
        ...
        function draw(ctx, cx, cy, wild, options)
        {
            ...
            var pixelSize = options.pixelSize !== undefined ? options.pixelSize : defaultPixelSize;
